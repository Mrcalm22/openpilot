...
class MiciHomeLayout(Widget):
  def __init__(self):
    ...
    self._turbopilot_label = UnifiedLabel("Turbopilot", font_size=96, font_weight=FontWeight.DISPLAY, max_width=480, wrap_text=False)
  ...
  def _render(self, _):
    ...
    self._turbopilot_label.set_position(text_pos.x, text_pos.y)
    self._turbopilot_label.render()

    if self._version_text is not None:
      # release branch
      release_branch = self._version_text[1] in RELEASE_BRANCHES
      version_pos = rl.Rectangle(text_pos.x, text_pos.y + self._turbopilot_label.font_size + 16, 100, 44)
